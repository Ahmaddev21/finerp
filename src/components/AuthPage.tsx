...
import { Mail, Lock, User, ArrowRight, Loader2, Building2, Shield, Users, CheckCircle2, Eye, EyeOff, TrendingUp, Truck, DollarSign, FileText, FolderOpen, Clock, UserCheck, ShieldCheck, BarChart2, Briefcase, Package } from 'lucide-react';
...
const MODULES = [
  { label: 'Delivery & Fleet',    desc: 'Riders, drivers & vehicles',   Icon: Truck,       from: '#3b82f6', to: '#1d4ed8', glow: 'rgba(59,130,246,0.4)'   },
  { label: 'Finance & Invoicing', desc: 'Revenue, expenses & P&L',      Icon: DollarSign,  from: '#10b981', to: '#047857', glow: 'rgba(16,185,129,0.4)'   },
  { label: 'Contracts',           desc: 'Workflow & approvals',          Icon: FileText,    from: '#8b5cf6', to: '#6d28d9', glow: 'rgba(139,92,246,0.4)'   },
  { label: 'Documents',           desc: 'QID, Istimara, Passport…',     Icon: FolderOpen,  from: '#f59e0b', to: '#b45309', glow: 'rgba(245,158,11,0.4)'   },
  { label: 'Time & Attendance',   desc: 'Clock-in, leaves & shifts',    Icon: Clock,       from: '#ef4444', to: '#b91c1c', glow: 'rgba(239,68,68,0.4)'    },
  { label: 'HR & Employees',      desc: 'Records & onboarding',         Icon: UserCheck,   from: '#06b6d4', to: '#0e7490', glow: 'rgba(6,182,212,0.4)'    },
  { label: 'Role & Access',       desc: 'Owner, admin & staff control', Icon: ShieldCheck, from: '#64748b', to: '#334155', glow: 'rgba(100,116,139,0.4)'  },
  { label: 'Reports & Analytics', desc: 'Charts, KPIs & exports',       Icon: BarChart2,   from: '#ec4899', to: '#be185d', glow: 'rgba(236,72,153,0.4)'   },
  { label: 'Payroll',             desc: 'Salaries & allowances',        Icon: Briefcase,   from: '#14b8a6', to: '#0f766e', glow: 'rgba(20,184,166,0.4)'   },
  { label: 'Inventory & Assets',  desc: 'Stock, tools & equipment',     Icon: Package,     from: '#f97316', to: '#c2410c', glow: 'rgba(249,115,22,0.4)'   },
];

/* ── Icon square with 3-D gradient + glow ─────────────── */
function ModuleIcon({ Icon, from, to, glow, size = 8 }: { Icon: React.ElementType; from: string; to: string; glow: string; size?: number }) {
  const px = size * 4;
  return (
    <span
      className={`w-${size} h-${size} rounded-xl flex items-center justify-center shrink-0`}
      style={{
        background: `linear-gradient(135deg, ${from}, ${to})`,
        boxShadow: `0 4px 12px ${glow}, 0 1px 3px rgba(0,0,0,0.15)`,
        width: px, height: px,
      }}
    >
      <Icon className="text-white" style={{ width: px * 0.44, height: px * 0.44 }} />
    </span>
  );
}
...
function DashboardMockup() {
  return (
    ...
    >
      {/* Main card */}
      <div
        className="w-[260px] bg-white rounded-2xl shadow-2xl overflow-hidden border border-slate-100"
        style={{ transform: 'rotateX(7deg) rotateY(-7deg) rotateZ(1deg)', transformStyle: 'preserve-3d' }}
      >
        <div className="bg-gradient-to-r from-[#2f3b9e] to-[#4a5bc7] px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white text-[10px] font-bold tracking-wide">FinERP Dashboard</span>
          </div>
        ...
        <div className="grid grid-cols-3 gap-px bg-slate-100 border-b border-slate-100">
          {[
            ...
            <div key={k.label} className="bg-white px-1.5 py-2 text-center">
              <p className={cn('text-[10px] font-black tabular-nums', k.color)}>{k.value}</p>
              <p className="text-[7px] text-slate-400 font-semibold uppercase tracking-wider mt-0.5">{k.label}</p>
              <p className={cn('text-[7px] font-bold mt-0.5', k.up ? 'text-emerald-500' : 'text-rose-400')}>
                {k.up ? '▲ +12%' : '▼ -3%'}
              </p>
            </div>
          ))}
        ...
        <div className="px-3.5 py-2.5 bg-white">
          <p className="text-[7px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Monthly Overview</p>
          <div className="flex items-end gap-0.5 h-8">
            {[40,65,45,80,60,90,55,75,85,70,95,72].map((h, i) => (
              <div key={i} className="flex-1 rounded-sm" style={{
                height: `${h}%`,
                background: i === 10 ? 'linear-gradient(to top,#2f3b9e,#4a5bc7)' : i >= 9 ? 'rgba(47,59,158,0.22)' : 'rgba(47,59,158,0.1)',
              }} />
            ))}
          </div>
          <div className="flex justify-between mt-1">
            {['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'].map(m => (
              <span key={m} className="text-[5px] text-slate-300 font-medium">{m.slice(0,1)}</span>
            ))}
          </div>
        </div>

        <div className="border-t border-slate-50 px-3.5 pb-2.5">
          <p className="text-[7px] font-bold uppercase tracking-wider text-slate-400 pt-2 mb-1.5">Recent Activity</p>
          {[
            ...
            <div key={r.name} className="flex items-center justify-between py-1 border-b border-slate-50 last:border-0">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[8px] text-slate-600 font-medium">{r.name}</span>
              </div>
              <span className={cn('text-[8px] font-bold', r.color)}>{r.amount}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div
        className="absolute top-0 right-2 bg-white rounded-xl border border-slate-100 px-2.5 py-1.5 flex items-center gap-1.5"
        style={{ transform: 'rotateX(7deg) rotateY(-7deg) translateZ(16px)', boxShadow: '0 8px 24px rgba(0,0,0,0.12)' }}
      >
        <div className="w-5 h-5 bg-emerald-100 rounded-lg flex items-center justify-center">
          <TrendingUp className="w-3 h-3 text-emerald-600" />
        </div>
        <div>
          <p className="text-[7px] text-slate-400 font-semibold leading-none">Net Profit</p>
          <p className="text-[10px] font-black text-emerald-600 leading-tight">+24.8%</p>
        </div>
      </div>

      {/* Floating chip — bottom left */}
      <div
        className="absolute bottom-0 left-2 bg-white rounded-xl border border-slate-100 px-2.5 py-1.5 flex items-center gap-1.5"
        style={{ transform: 'rotateX(7deg) rotateY(-7deg) translateZ(16px)', boxShadow: '0 8px 24px rgba(0,0,0,0.12)' }}
      >
        ...
        <div>
          <p className="text-[7px] text-slate-400 font-semibold leading-none">Fleet Active</p>
          <p className="text-[10px] font-black text-blue-600 leading-tight">48 Riders</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="h-screen flex overflow-hidden bg-slate-50">
      ...
      <div
        ...
        style={{ background: 'linear-gradient(160deg,#ffffff 0%,#f0f4ff 55%,#e8eeff 100%)' }}
      >
        {/* Decorative blobs */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute -top-16 -right-16 w-80 h-80 bg-blue-100/50 rounded-full blur-[90px]" />
          <div className="absolute -bottom-16 -left-16 w-96 h-96 bg-indigo-100/40 rounded-full blur-[90px]" />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-violet-100/30 rounded-full blur-[70px]" />
        </div>

        <div className="relative z-10 flex flex-col h-full px-10 pt-7 pb-5 gap-5">

          {/* ── Logo ── */}
          <div className="flex flex-col items-start gap-1.5 shrink-0">
            <div className="w-11 h-11 rounded-2xl bg-white shadow-md border border-slate-100 flex items-center justify-center p-2 mb-0.5">
            ...
            <p className="text-[9.5px] font-black text-[#2f3b9e]/75 tracking-[0.06em] uppercase leading-snug">
          ...
          <div className="shrink-0">
            <h1 className="text-[28px] font-black text-slate-900 leading-[1.1] tracking-tight">
              Smart Operations.<br />
              <span className="text-[#2f3b9e]">One Platform.</span>
            </h1>
            <p className="text-slate-500 text-[11.5px] mt-1.5 leading-relaxed max-w-xs">
              Everything your team needs — finance, delivery, contracts, HR &amp; documents — in a single workspace built for Rafi Al Aftab.
            </p>
          </div>

          {/* ── Main content: modules (left) + mockup (right) ── */}
          <div className="flex-1 min-h-0 flex gap-5">

            {/* Module grid — 2 columns × 5 rows */}
            <div className="flex flex-col justify-between w-[52%] shrink-0">
              <div className="grid grid-cols-2 gap-2 h-full content-start">
                {MODULES.map(({ label, desc, Icon, from, to, glow }) => (
                  <div
                    key={label}
                    className="flex items-start gap-2.5 p-2.5 bg-white/70 rounded-xl border border-white/90 backdrop-blur-sm"
                    style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06), 0 0 0 1px rgba(255,255,255,0.8)' }}
                  >
                    <ModuleIcon Icon={Icon} from={from} to={to} glow={glow} size={7} />
                    <div className="min-w-0">
                      <p className="text-[10.5px] font-bold text-slate-800 leading-tight truncate">{label}</p>
                      <p className="text-[9px] text-slate-400 leading-tight mt-0.5 truncate">{desc}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="pt-3 border-t border-slate-200/70 flex items-center justify-between shrink-0">
            <p className="text-[10px] font-semibold text-slate-500">© 2025 Rafi Al Aftab Trading &amp; Contracting Co. W.L.L</p>
            <p className="text-[10px] font-semibold text-slate-400">Qatar · All rights reserved</p>
          </div>
        </div>
      </div>
          ...
          {mode === 'forgot-password' ? (
            resetSent ? (
              ...
              <form onSubmit={handleForgotPassword} className="space-y-4">
                ...
                <button type="submit" disabled={loading} className="w-full font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 text-sm text-white active:scale-[0.98] transition-all"
                  style={{ background: 'linear-gradient(135deg,#2f3b9e,#4a5bc7)' }}>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                {mode === 'signup-join' && (
                  <div className="relative">
                    ...
                    <input type="text" placeholder="INVITE CODE" value={form.joinCode}
                ...
                <button type="submit" disabled={loading}
                  className="w-full disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold text-sm py-3.5 rounded-xl flex items-center justify-center gap-2 mt-1 active:scale-[0.98] transition-all"
                  style={{ background: 'linear-gradient(135deg,#2f3b9e,#4a5bc7)', boxShadow: '0 8px 24px rgba(47,59,158,0.25)' }}
                >
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (
                    <>{mode === 'login' ? 'Sign in' : mode === 'signup-create' ? 'Create company' : 'Join team'}<ArrowRight className="w-4 h-4" /></>
                  )}
                </button>
              </form>
